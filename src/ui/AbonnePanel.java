...
/**
 * AbonnePanel - Gestion des abonnés (thème clair, boutons en bas)
 */
public class AbonnePanel extends JPanel {

    private JTable tableau;
    private DefaultTableModel modeleTableau;
    private JTextField champNom, champAdresse, champDateAbon, champDateEntree;
    ...
    public AbonnePanel() {
        ...
        JLabel labelTitre = new JLabel("\uD83D\uDC64  Gestion des Abonnés");
        ...
        // Formulaire ligne 1
        JPanel formLigne1 = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 4));
        formLigne1.setBackground(COULEUR_CARTE);
        formLigne1.add(creerLabel("Nom :"));
        champNom = creerChamp(15);
        formLigne1.add(champNom);
        formLigne1.add(creerLabel("Adresse :"));
        champAdresse = creerChamp(20);
        formLigne1.add(champAdresse);

        // Formulaire ligne 2
        JPanel formLigne2 = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 4));
        formLigne2.setBackground(COULEUR_CARTE);
        formLigne2.add(creerLabel("Date abonnement :"));
        champDateAbon = creerChamp(10);
        formLigne2.add(champDateAbon);
        formLigne2.add(creerLabel("Date entrée :"));
        champDateEntree = creerChamp(10);
        formLigne2.add(champDateEntree);

        JPanel panneauForm = new JPanel();
        panneauForm.setLayout(new BoxLayout(panneauForm, BoxLayout.Y_AXIS));
        ...
        panneauForm.setMaximumSize(new Dimension(Integer.MAX_VALUE, 110));
        panneauForm.add(formLigne1);
        panneauForm.add(formLigne2);

        panneauNord.add(panneauForm);
        add(panneauNord, BorderLayout.NORTH);

        // Tableau
        String[] colonnes = {"N° Abonné", "Nom", "Adresse", "Date Abonnement", "Date Entrée", "Nb Locations"};
        ...
        btnAjouter.addActionListener(e -> {
            if (champNom.getText().trim().isEmpty()) { JOptionPane.showMessageDialog(this, "Le nom est obligatoire !"); return; }
            Abonne ab = new Abonne();
            ab.setNomAbonne(champNom.getText().trim());
            ab.setAdresseAbonne(champAdresse.getText().trim());
            ab.setDateAbonnement(champDateAbon.getText().trim());
            ab.setDateEntree(champDateEntree.getText().trim());
            if (abonneDAO.ajouter(ab)) {
        ...
        btnModifier.addActionListener(e -> {
            ...
            if (l == -1) { JOptionPane.showMessageDialog(this, "Sélectionnez un abonné !"); return; }
            if (champNom.getText().trim().isEmpty()) { JOptionPane.showMessageDialog(this, "Le nom est obligatoire !"); return; }
        ...
        btnSupprimer.addActionListener(e -> {
            ...
            if (l == -1) { JOptionPane.showMessageDialog(this, "Sélectionnez un abonné !"); return; }
            int num = (int) modeleTableau.getValueAt(l, 0);
            int choix = JOptionPane.showConfirmDialog(this, "Supprimer ?", "Confirmation", JOptionPane.YES_NO_OPTION);
            if (choix == JOptionPane.YES_OPTION) {
                if (abonneDAO.supprimer(num)) {
                    JOptionPane.showMessageDialog(this, "Supprimé !");
                    viderFormulaire(); chargerDonnees();
                } else {
                    JOptionPane.showMessageDialog(this, "Impossible : locations en cours.", "Erreur", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
        ...
        tableau.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                int l = tableau.getSelectedRow();
                if (l >= 0) {
                    champNom.setText(String.valueOf(modeleTableau.getValueAt(l, 1)));
                    champAdresse.setText(String.valueOf(modeleTableau.getValueAt(l, 2)));
                    champDateAbon.setText(String.valueOf(modeleTableau.getValueAt(l, 3)));
                    champDateEntree.setText(String.valueOf(modeleTableau.getValueAt(l, 4)));
                }
            }
        });
        chargerDonnees();
    ...
    private void chargerDonnees() {
        ...
        for (Abonne ab : abonneDAO.listerTout()) {
            modeleTableau.addRow(new Object[]{ ab.getNumAbonne(), ab.getNomAbonne(), ab.getAdresseAbonne(),
                ab.getDateAbonnement(), ab.getDateEntree(), ab.getNbLocation() });
        }
    }
    private void viderFormulaire() {
        champNom.setText(""); champAdresse.setText(""); champDateAbon.setText(""); champDateEntree.setText("");
